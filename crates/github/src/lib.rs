...
#[graphql(
    schema_path = "src/schema.graphql",
    query_path = "src/resolve_project_owner_context.graphql",
    response_derives = "Debug, Serialize, Deserialize"
)]
pub struct ResolveProjectOwnerIssueContext;
...
impl GithubIssueTracker {
    ...
    async fn project_context(&self, issue: &Issue) -> Result<Option<ProjectContext>, CoreError> {
        ...
        let issue_number = issue
            ...
            .parse::<u64>()
            .map_err(|error| CoreError::Adapter(error.to_string()))?;
        // `user(login:)` and `organization(login:)` can each return a
        // GraphQL error for the opposite owner type. `repositoryOwner` is the
        // owner-type-safe lookup and resolves either form without making an
        // invalid second lookup.
        let response = self
            .graphql::<resolve_project_owner_issue_context::ResponseData, _>(
                ResolveProjectOwnerIssueContext::build_query(
        ...
        let project_id = project_id_from_owner_context(&data).ok_or_else(|| {
            CoreError::Adapter(format!(
                "github project {}/{} not found for repository owner",
